...
for f in "$APP" "$MODS" "$SR" "$MNT/README.txt" "$MNT/BUILD-INFO.txt" "$MNT/Fix Launch Problems.command"; do
	[ -e "$f" ] && ok "present: $(basename "$f")" || bad "missing: $(basename "$f")"
done
[ -x "$MNT/Fix Launch Problems.command" ] && ok "Fix Launch Problems.command is executable" \
                                     || bad "Fix Launch Problems.command is not executable"
