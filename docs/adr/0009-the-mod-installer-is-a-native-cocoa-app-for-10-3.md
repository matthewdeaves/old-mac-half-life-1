...
x86_64, then `lipo`s the two together (`:57-59`, header at `:11-21`).
...
nib (`installer/README.md:62-68`). Several things had to be written: `OMTGA.m`
...
Leopard mis-grade, and a plain `[ppc, x86_64]` fat is the ordinary 2006 case
...
- One binary, one code path, on a G3 under 10.3.9 and an Intel mini under 10.7.5.
...
was pinned to an explicit size (`installer/README.md:112-118`).
