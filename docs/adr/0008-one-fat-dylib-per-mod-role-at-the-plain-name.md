...
**Build each mod's code twice, `lipo` the slices into one fat `ppc + x86_64` file
per role, and install it at the plain name from `liblist.gam`.**

- `scripts/build-mod.sh:365-366` produces fat `server.dylib` and `client.dylib`.
...
- One generic `ppc` slice covers the G3, G4 and G5: game code is loaded with
  ...
  the engine executable needs an exact cpusubtype (`scripts/build-mod.sh:41-45`,
...
- Builds are keyed by hlsdk branch with generic filenames; the installer names
  ...
  what it put on disk (`scripts/build-mod.sh:17-28`, `docs/MODS.md:167-181`).

## Alternatives rejected

- **Arch-suffixed files, as Xash intends.** Four game dylibs per mod folder, 100
  files across 25 mods, and installing a mod stops being an overwrite of what its
  own release ships.
...
- **One server dylib reused across mods.** `eftd` renumbers the `func_breakable`
  ...
  for `spawnobject 12` gets an RPG instead of the .357 (`docs/MODS.md:183-195`).
...
- One file per role per mod, loading on the G3, G4, G5 and Intel, installed as a
  straight overwrite of the filename the mod's own release already uses on any
  platform. Nothing changes for the suffixed names or the existing `valve/`
  layout unless the suffixed lookup fails.
...
- Risk: `installer/mods.map` is the only link from a content gamedir to a build
  ...
  (`.claude/rules/shipped-layout.md:46-50`).
...
which happened so a genuine failure is still reported (`docs/MODS.md:56-100`).
