...
them (`scripts/make-app.sh:161-167`).
...
- The engine mounts `Contents/Resources/Half-Life` as its read-only root
  ...
  the player's folder (`scripts/make-app.sh:92-93`).
...
`scripts/deploy-dmg.sh:113-118` still removes those leftovers on the bench
machines.
...
- With no `valve/` folder the failure is deep inside filesystem init, on a console
  ...
  first and says so in a dialog (`scripts/make-app.sh:119-124`), a workaround for
